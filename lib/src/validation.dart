...
void validateSocketId(String socketId) {
  if (socketId != null && SOCKET_ID_REGEX.hasMatch(socketId) == false) throw new FormatException(
...
void validateChannelName(String name) {
  if (name.length > CHANNEL_NAME_MAX_LENGTH) throw new ArgumentError(
      "The length of the channel name was greater than the allowed ${CHANNEL_NAME_MAX_LENGTH} characters");
  else if (CHANNEL_NAME_REGEX.hasMatch(name) == false) throw new FormatException(
